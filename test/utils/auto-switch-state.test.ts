...
test('formatRateLimitWindowLabel formats common and custom windows', () => {
  ...
  assert.equal(
    formatRateLimitWindowLabel({ ...base, windowDurationMins: 300 }),
    '5h',
  )
  assert.equal(
  ...
  assert.equal(
    formatRateLimitWindowLabel({ ...base, windowDurationMins: 2880 }),
    '2d',
  )
  assert.equal(
    formatRateLimitWindowLabel({ ...base, windowDurationMins: 120 }),
    '2h',
  )
  assert.equal(
    formatRateLimitWindowLabel({ ...base, windowDurationMins: 45 }),
    '45m',
  )
})
...
test('describeThresholdReason includes every triggering window', () => {
  assert.equal(
    describeThresholdReason(profile('a', 99, 100), 99),
    '5h 99% + weekly 100%',
  )
...
test('isHysteresisBlocked releases after reset or recovery', () => {
  ...
  assert.equal(
    isHysteresisBlocked(profile('a', 95, 20), state, 90, 2000),
    false,
  )
  assert.equal(
    isHysteresisBlocked(profile('b', 95, 20), state, 90, 1000),
    false,
  )
  assert.equal(
    isHysteresisBlocked(profile('a', null, null), state, 90, 1000),
    true,
  )
  assert.equal(
    isHysteresisBlocked(profile('a', 95, 20), undefined, 90, 1000),
    false,
  )
})
...
test('findNextResetAt returns nearest future reset', () => {
  assert.equal(
    findNextResetAt(
      [profile('a', 100, 100), profile('b', 100, 100, 1500, 4000)],
      1000,
    ),
    1500,
  )
  assert.equal(
    findNextResetAt([profile('a', 100, 100, 500, 900)], 1000),
    undefined,
  )
})
